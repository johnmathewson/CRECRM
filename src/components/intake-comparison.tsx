...
export default function IntakeComparison({ units, propertyName }: Props) {
  ...
  const savePendingComps = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/comps/save", {
        ...
        body: JSON.stringify({ comps: pendingComps, source: "upload" }),
      });
  ...
  }, [pendingComps, loadComps]);

  // ── Comparison logic ─────────────────────────────────────
  // Intake averages (occupied units only)
  ...
  // Comp averages (all comps with lease_rate)
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* ── Comp Upload Section ── */}
      <Panel
        title={`Comps Database — ${comps.length} records`}
        actions={
          <div className="flex gap-1 items-center">
            <button
              onClick={() => setShowUpload(!showUpload)}
              className="px-2.5 py-1 text-[10.5px] font-semibold cursor-pointer"
              style={{
                borderRadius: 4,
                border: "none",
                fontFamily: "inherit",
      ...
      >
            ...
            {pendingComps.length > 0 ? (
              <div>
                <div className="text-[12px] font-medium text-cream mb-2">
                  {pendingComps.length} comps extracted — review and save:
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {savingComps ? "Saving..." : `Save ${pendingComps.length} Comps`}
                  </button>
              ...
              <div>
                <div className="text-[12px] text-cream-muted mb-2">
                  Upload a file with comparable lease data (PDF, Excel, CSV, Word, or image)
                </div>
                <button
                  onClick={() => fileRef.current?.click()}
                  disabled={uploading}
                  style={{ ...btnSecondary, fontSize: 11, padding: "6px 14px" }}
                >
        ...
        {loading ? (
          ...
          <div className="text-cream-subtle text-[12px] py-4 text-center">No comps yet — upload your first comp file</div>
        ) : (
          <div className="overflow-x-auto" style={{ maxHeight: 220 }}>
            <table className="w-full" style={{ borderCollapse: "separate", borderSpacing: "0 2px" }}>
              <thead>
                <tr className="text-[9.5px] text-cream-subtle uppercase tracking-wider">
                  {["Property", "Tenant", "SF", "Rate/SF", "Type", "City", "Date"].map((h, i) => (
              ...
              <tbody>
                {comps.slice(0, 15).map((c) => (
                  <tr key={c.id}>
                    ...
                    <td className="px-2 py-1.5 text-[11px] text-cream-muted" style={{ background: "rgba(255,255,255,0.02)" }}>{c.city || "—"}</td>
                    <td className="px-2 py-1.5 text-[10px] text-cream-subtle" style={{ background: "rgba(255,255,255,0.02)", borderRadius: "0 4px 4px 0" }}>
      ...
      {/* ── Delta Analysis ── */}
      {compsWithRate.length > 0 && occupiedUnits.length > 0 && (
        <>
          {/* Summary delta cards */}
          <Panel title="Market Comparison">
            ...
            {/* Revenue impact */}
            <div className="glass-inner p-3.5 mb-3" style={{ borderLeft: `2px solid ${revenueDelta > 0 ? C.green : C.coral}` }}>
          ...
          <Panel title="Unit-by-Unit Market Analysis" actions={
            <span className="text-[10.5px] text-cream-subtle mr-2">vs. {compsWithRate.length} comps</span>
          }>
