 ...
 *
 * North-star contract: one relationship in one view (email + texts + calls
 * interleaved), reply in-channel, AI draft pre-loaded for email. All sends
 * go through the EXISTING lead routes (/send, /send-sms, /draft) — those
 * stamp final_sent_at / status='sent' and write the outbound comms row,
 * which is what clears Unanswered everywhere. No new send paths.
 */
...
export function ThreadPanel({
  ...
  // Pick the reply channel once the lead arrives: reply in kind, fall back
  // to whichever address we actually have.
  useEffect(() => {
    if (!data || replyChannel) return;
    const hasPhone = !!data.lead.sender_phone;
    const hasEmail = !!data.lead.sender_email;
  ...
  useEffect(() => {
    ...
    if (replyChannel === "email" && data.lead.draft_reply && !text) {
      setText(data.lead.draft_reply);
      setDraftLoaded(true);
    }
  ...
  function switchChannel(ch: ReplyChannel) {
    ...
    if (ch === "email" && data?.lead.draft_reply) {
      setText(data.lead.draft_reply);
      setDraftLoaded(true);
    } else {
      setText("");
  ...
  async function regenerateDraft() {
    ...
    try {
      ...
      if (fresh) setText(fresh);
        ...
        const res2 = await fetch(`/api/leads/${leadId}`);
  ...
  const who =
    data?.lead.contact?.full_name ||
    data?.lead.sender_name ||
    data?.lead.sender_email ||
    data?.lead.sender_phone ||
    "Unknown";
    ...
    <div className="fixed inset-0 z-40" role="dialog" aria-modal="true">
      ...
      <div className="absolute inset-x-0 bottom-0 top-14 lg:inset-y-0 lg:left-auto lg:right-0 lg:w-[480px] flex flex-col bg-steward-base border-t lg:border-t-0 lg:border-l border-white/10 rounded-t-xl lg:rounded-none shadow-2xl">
        ...
        <div className="flex items-start justify-between gap-3 px-4 py-3 border-b border-white/[0.06]">
          <div className="min-w-0">
            ...
            <div className="mt-0.5 flex items-center gap-2 font-mono text-[10px] text-cream-subtle">
              {data?.lead.property?.name && (
                <span className="px-1.5 py-0.5 rounded-full bg-teal-400/[0.08] border border-teal-400/25 text-teal-300">
                  {data.lead.property.name}
                </span>
              )}
              <a href={`/cre-os/inbox/${leadId}`} className="underline underline-offset-2 hover:text-cream-dim">
                Open lead file →
              </a>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
          ...
          {data && data.thread.length === 0 && (
            <p className="font-body text-[12px] text-cream-subtle py-4">
              No messages logged yet on this lead.
            </p>
          )}
          {data?.thread.map((m) => {
        ...
        <div className="border-t border-white/[0.06] px-4 py-3 space-y-2 [padding-bottom:max(0.75rem,env(safe-area-inset-bottom))]">
          ...
          {data && !data.lead.sender_phone && !data.lead.sender_email ? (
            ...
            <>
              <div className="flex items-center gap-2">
                {data?.lead.sender_phone && (
                  <ChannelTab
                    label={`Text ${data.lead.sender_phone}`}
                ...
                {data?.lead.sender_email && (
                  <ChannelTab
                    label={`Email ${data.lead.sender_email}`}
