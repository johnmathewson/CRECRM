...
| `/api/leads/[id]/send` | Sends final approved reply | Reads possibly-edited `draft_reply`, sends via Gmail API from the connected primary mailbox (inquiries@), updates `final_reply`, `final_sent_at`, `status='sent'`. Authenticated. |
...
- **Google Workspace:** mailbox `inquiries@stewardshipcre.com` being provisioned (separate from John's personal mailbox to keep AI watcher's signal clean — no classification step needed)
...
- **Gmail API for sending:** use the Workspace Gmail API with domain-wide delegation OR OAuth on the sending mailbox (currently inquiries@; John's real personal address is john@johnmathewson.co — **john@stewardshipcre.com does NOT exist**)
...
**New Communications stream** at `/cre-os/stream` (nav: "Communications") — north-star surface: chronological, day-grouped, filter chips (Unanswered / channel / property / People-only), read-only over `communications`. **7/29: ThreadPanel shipped** — tap a row w/ lead → bottom-sheet/side-panel thread (email+SMS+calls interleaved via GET /api/leads/[id]) + channel-aware reply bar (Text/Email tabs, replies in kind, AI draft pre-loads for email w/ regenerate). All sends via existing lead routes (/send, /send-sms, /draft) so final_sent_at/status/Unanswered stay in sync. **Panel is dual-mode (7/29 late):** contact-only rows (no lead) open it too via GET /api/contacts/[id]/thread; first send POSTs /api/contacts/[id]/ensure-lead (source='other', status='contacted', adopts orphaned comms — outbound outreach, never queues/auto-acks) then uses the normal lead routes. **Inbox tab RETIRED from Sidebar + BottomNav 7/29** (John approved after testing the panel); "Comms log" renamed "Outreach tracker" (it's reply-rate/touch-kind/per-property analytics, NOT a duplicate of the stream — don't merge it away). The `/cre-os/inbox` list page + `/cre-os/inbox/[id]` lead files remain ROUTABLE — all SMS deeplinks + in-app lead links depend on [id]; Home/Prospector still link to the old list until Today absorbs Home. Note: Home's `?bucket=hot` param was always ignored by InboxView (known dead param). Design docs (in ~/Documents/Claude/Projects/Commercial Real Estate/): `CRM-North-Star-Architecture.md` (the model: two nouns, one stream, Unanswered = only status, AI draft pre-loaded in reply bar), `CRM-Full-UI-Audit.md` (per-screen verdicts, 16→8 nav, metrics disagreements), `Inbox-Overhaul-Diagnosis.md`, `A2P-SUBMIT-THIS.md`.

**Readability pass (done):** cream tiers now dim 86% / muted 72% / subtle 62% (tailwind.config). `.canvas-light` retargets in globals.css: accent -300/-400 text → deep hues; opacity-40/50/60 clamped to 0.72. ROOT CAUSE was whole-element opacity fades stacking on pastel text — check for that pattern before touching colors again. `metrics.ts` **WIRED 7/29**: countHotLeads()/countActiveListings() now feed Home dashboard + copilot chips, Listings roster, Reports listing-reach; hot excludes converted; STAGE_ORDER re-exports stage-config PIPELINE_LADDER (stage-config is the one stage vocabulary — both Pipeline + Reports normalize through it). **Gmail is multi-mailbox as of 7/29**: getActiveGmailToken pins to GMAIL_PRIMARY_MAILBOX (default inquiries@) — inbound polling + sends stay there; SENT-sync loops ALL connected accounts (raw_payload.mailbox = provenance). **John's real mailbox (john@johnmathewson.co — john@stewardshipcre.com does NOT exist) still needs its one-time OAuth connect** (Settings → Google connect signed into that account; may need adding as OAuth test user if Google blocks) — until then his direct Gmail replies are invisible; after connect, sent-sync auto-backfills 21 days, older gaps via POST /api/communications/coverage (primary-mailbox-only for now).
...
**7/29 (2nd half) — SHIPPED:** queue items 1-4 done (voice+notifications, metrics wiring, lead dedupe, ThreadPanel). Also: **intake dedupe** (repeat inquiries thread into open lead — same email+property, 30-day window, both /api/leads/intake + /api/public/questionnaire; no auto-ack on repeats; 48 dup leads merged into 37 keepers in prod, children repointed across 10 FK tables, dups archived w/ notes). **Person-scoped threads** (GET /api/leads/[id] pulls ALL comms for the contact — dup-lead scatter made replies invisible). **Unanswered honesty**: 81 phantom parties were 'CREXi activity' signal rows from a backfill under source='lead_comm_backfill' — ~230 rows relabeled to crexi_report in prod + isSignal() (source OR subject prefix) hardening in comms-stream-queries; count went 106 → ~25 true. **Coverage backfill is multi-mailbox** (?mailbox=/body.mailbox; non-primary = outbound-only + known-contact/lead-only + skipped_personal ledger in email_ingest_log so batches advance). **john@johnmathewson.co CONNECTED 7/29 ~16:28Z** — live sent-sync working (3 rows immediately); 90-day deep backfill STARTED: dry-run showed 311 sent / ~130 personal per 150; first real batches may still be pending (a tool outage interrupted — verify `SELECT count(*) FROM communications WHERE raw_payload->>'mailbox'='john@johnmathewson.co'` and re-POST /api/communications/coverage {days:90, mailbox:'john@johnmathewson.co', limit:150} until remaining=0). NOTE: live SENT-sync only scans newest-50/run — deep history is coverage's job. **Al Dziadkowiec ×4 contacts** (same name, 4 different phones, 1 email, no shared keys) — needs John's judgment before merging.

**Build queue (remaining):** 5) contact form + consent columns + sms_optouts (marketing-site form STILL a stub), 6) Today absorbs Home; Properties absorbs Listings (Inbox retired 7/29; Comms log renamed Outreach tracker); Prospector restructure (see agents plan below).

**AGENTS ARCHITECTURE (agreed 7/29 late — John's spec):** Four new agents planned: (1) Property Enricher, (2) Relationship/Contact Enricher, (3) Communication Agent (mines both mailboxes for drifting relationships + talking points), (4) Campaign Prospector (declarative campaigns — "every medical office + owner + how they bought" — replaces the 4 draft lanes; existing cadence/personas/voice become its execution layer). Steward + existing watchers/crons FROZEN per John. Shared spine: completeness scorecards, enrichment_requests work queue (campaigns push, enrichers pull), field-level provenance, human-data-wins, visible per-run diffs, budgets. Two tiers: cloud (Netlify crons, DB+open web) + desktop (Claude Code driving John's logged-in Chrome for CoStar/PropStream — record-level lookups only, human pace; CoStar is scraping-litigious, never bulk-harvest). NOTE: properties table = 15,130 rows (cold inventory lives THERE, not a separate table); assessor_records/parcels/beneficial_owners/leases/units/buildings all EMPTY scaffolding; sale_comps 1,853 + lease_comps 438 are the real cross-ref sources.

**Property Enricher v1 SHIPPED 7/29 late (agent #1):** `src/lib/agents/property-enricher.ts` — DB-only daily (Netlify 08:00 UTC, batch 40, route /api/agents/property-enricher/run POST cron-secret|session, GET = last 10 run summaries). Passes: owner canonicalization (true>recorded>raw + entity regex + E.164) · sale-comp address cross-match (fill blanks, conflict facts, Haiku fuzzy adjudication cap 15/run) · portfolio detection · owner→contacts linking (phone=high fills owner_contact_id) · completeness score (10-field scorecard). Tables (migration 0033): property_facts (UNIQUE property_id,field,source; '_scanned' fact = 14-day rotation cursor), enrichment_runs (visible diff), enrichment_requests (shared queue, priority asc). First prod run verified. Batch selection: queue > working set (real statuses) > cold-15k by updated_at. To crank throughput later: raise schedule frequency, not batch size (60s function cap).
